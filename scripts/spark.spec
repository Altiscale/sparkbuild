...
if [ "$1" = "0" ]; then
  ...
  if [ "x${ret}" != "x0" ] ; then
    ...
    echo "ok - uninstalling %{rpm_package_name}-devel on system, removing everything related except the parent directoy /opt/%{spark_folder_name}"
    rm -vrf %{install_spark_dest}/core
    rm -vrf %{install_spark_dest}/sql/catalyst
    rm -vrf %{install_spark_dest}/sql/core
    rm -vrf %{install_spark_dest}/sql/hive
...
if [ "$1" = "0" ]; then
  ...
  if [ "x${ret}" != "x0" ] ; then
    ...
    echo "ok - uninstalling %{rpm_package_name}-kinesis on system, removing everything related except the parent directoy /opt/%{spark_folder_name}"
    rm -vrf %{install_spark_dest}/extras/kinesis-asl
    rm -vrf %{install_spark_dest}/extras/kinesis-asl-assembly
  fi
fi
...
- Remove spark example subpkg and test case
