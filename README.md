...
(Spark 1.6.1 was the only exception due to legacy, and for Spark2 onward, we will follow this pattern)
...
su - makerpm
cd $HOME
git clone -b branch-2.1-alti https://github.com/Altiscale/sparkbuild.git
pushd sparkbuild
git clone -b branch-2.1-alti git@github.com:Altiscale/spark.git
# git clone -b branch-2.1-alti https://github.com/Altiscale/spark.git
```
...
INCLUDE_LEGACY_TEST=false SPARK_BRANCH_NAME=branch-2.1-alti ./build_rpm_only.sh
```
...
==========
```
# Install on Hadoop 2.7.1 for Spark version x.y.z (e.g. x.y.z could be 1.6.1/1.6.2/2.0.2/2.1.0/etc.)
...
accordingly for that specific Spark version. For example, to run Spark 2.1.0
...
```
SPARK_HOME=/opt/alti-spark-2.1.0
SPARK_CONF_DIR=/etc/alti-spark-2.1.0
cp -rp /opt/alti-spark-2.1.0/test_spark /tmp/test_spark-2.1.0
cd /tmp/test_spark-2.1.0/
# For non-Kerberos cluster
./run_all_test.nokerberos.sh
# For kerberos enabled cluster
./run_all_test.kerberos.sh
