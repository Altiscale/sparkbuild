...
SPARK_EXAMPLE_JAR=$(find ${spark_home}/examples/target/scala-${SPARK_SCALA_VERSION}/jars/spark-examples_*-${spark_version}.jar)
...
./bin/spark-submit --verbose --master yarn --deploy-mode cluster $queue_name $spark_shuffle_conf --conf spark.eventLog.dir=${spark_event_log_dir}/$USER --class org.apache.spark.examples.SparkPi "${SPARK_EXAMPLE_JAR}" 3000
